...
public class WechatWorkIdentityProvider extends AbstractOAuth2IdentityProvider<WechatWorkProviderConfig> implements SocialIdentityProvider<WechatWorkProviderConfig> {
    ...
    // 企业微信外部使用
    private static final String QRCODE_AUTH_URL = "https://open.work.weixin.qq.com/wwopen/sso/qrConnect";
    ...
    private static final String WEIXIN_REDIRECT_FRAGMENT = "wechat_redirect";

    private static final String PROFILE_URL = "https://qyapi.weixin.qq.com/cgi-bin/user/getuserinfo";
    private static final String PROFILE_DETAIL_URL = "https://qyapi.weixin.qq.com/cgi-bin/user/get";

    private static final String OAUTH2_PARAMETER_CLIENT_ID = "appid";
    private static final String OAUTH2_PARAMETER_AGENT_ID = "agentid";
    private static final String OAUTH2_PARAMETER_RESPONSE_TYPE = "response_type";

    private static final String WEIXIN_CORP_ID = "corpid";
    private static final String WEIXIN_CORP_SECRET = "corpsecret";
    ...
    private static final DefaultCacheManager CACHE_MANAGER = new DefaultCacheManager();
    private static final String WECHAT_WORK_CACHE_NAME = "wechat_work_sso";
    private static final ConcurrentMap<String, Cache<String, String>> CACHES = new ConcurrentHashMap<>();

    /**
     * 构造器
     */
    WechatWorkIdentityProvider(KeycloakSession session, WechatWorkProviderConfig config) {
    ...
    private static Cache<String, String> createCache(String suffix) {
        try {
            ...
            CACHE_MANAGER.defineConfiguration(cacheName, config.build());
            Cache<String, String> cache = CACHE_MANAGER.getCache(cacheName);
            logger.info(cache);
            return cache;
        } catch (Exception e) {
            logger.error(e);
    ...
    private Cache<String, String> getCache() {
        return CACHES.computeIfAbsent(getConfig().getClientId() + ":" + getConfig().getAgentId(), WechatWorkIdentityProvider::createCache);
    }

    // 从缓存获取AccessToken，没有则请求接口
